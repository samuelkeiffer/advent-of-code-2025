...
fn part2(file: &str) -> u64 {
    ...
    banks.iter().map(|bank| {
        let mut index = 0;
        (0..12).rev().map(|digit| {
            let (i, joltage) = bank.iter().enumerate().take(bank.len() - digit).skip(index).max_by_key(|(i, x)| **x as i32 * 1000 - *i as i32).unwrap();
            index = i + 1;
...
fn part1(file: &str) -> u32 {
    let banks = parse_file(file);
    banks.iter().map(|bank| {
        let (i, first) = bank.iter().enumerate().take(bank.len() - 1).max_by_key(|(i, x)| **x as i32 * 1000 - *i as i32).unwrap();
